...
public class GestureLockViewGroup extends RelativeLayout {
    ...
    /**
     * GestureLockView的边长为 4 * mWidth / ( 5 * mCount + 1 )
     */
     ...
     */
    private int mNoFingerColor = 0xFFBBBBBB;

    /**
     * GestureLockView手指触摸的状态下圆的颜色
     */
    private int mFingerOnColor = 0XFF3AA6E8;
    /**
     * GestureLockView手指抬起的状态下,正确时圆的颜色
     */
    private int mFingerUpColorCorrect = 0XFF3AA6E8;
    ...
    public GestureLockViewGroup(Context context, AttributeSet attrs, int defStyle) {
         ...
         * 获取密码状态
         */
        password = SPManager.getGesturePassword();

        Log.d(TAG, "password now is : " + password);
        isSetPassword = !"".equals(password); //判断是否已经保存有密码
        isInPasswordSettingMode = !isSetPassword;     //当未设置密码，进入密码设置模式

        // 初始化画笔
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaint.setStyle(Paint.Style.STROKE);
        mPaint.setStrokeCap(Paint.Cap.ROUND);//画笔带圆角
        mPaint.setStrokeJoin(Paint.Join.ROUND);//转角圆滑过渡
        mPath = new Path();
    }
    ...
    private void initViews() {
        ...
        if (mGestureLockViews == null) {
            ...
            for (int i = 0; i < mGestureLockViews.length; i++) {
                ...
                if (mFingerOnColor == 0X00ffffff) {
                    mFingerOnColor = mNoFingerColor;
                    mFingerUpColorCorrect = mNoFingerColor;
                    mFingerUpColorError = mNoFingerColor;
                }
...
//                    }
    ...
    public void setGestureUnmatchedExceedListener(GestureUnmatchedExceedListener gestureUnmatchedExceedListener) {
    ...
    public void savePassword(String password) {
        ...
        SPManager.putGesturePassword(password);
    }
    ...
    public void setPaintColorAndAlpha(int color, int alpha) {
        if (mGestureLockViews != null) {
            for (GestureLockView mGestureLockView : mGestureLockViews) {
                mGestureLockView.setColorFingerOn(color);
            }
        }

        mFingerOnColor = color;
        mPaintAlpha = alpha;
