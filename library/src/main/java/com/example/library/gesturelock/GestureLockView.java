...
public class GestureLockView extends View {
     ...
     */
    private int mColorNoFinger;//无手指触摸的状态下圆的颜色
    private int mColorFingerOn;//外圆颜色
    ...
    public GestureLockView(Context context, int colorNoFingerr, int colorFingerOn, int colorCorrect, int colorError) {
        ...
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);//Paint.ANTI_ALIAS_FLAG抗锯齿
        mArrowPath = new Path();
    }
     ...
     * 设置当前手势颜色
     *
     * @param mColorFingerOn
     */
    public void setColorFingerOn(int mColorFingerOn) {
        if (mColorFingerOn == 0X00ffffff) {
            this.mColorFingerOn = mColorNoFinger;
            this.mColorFingerUpCorrect = mColorNoFinger;
            this.mColorFingerUpError = mColorNoFinger;
        } else {
            this.mColorFingerOn = mColorFingerOn;
            this.mColorFingerUpCorrect = mColorFingerOn;
            this.mColorFingerUpError = mColorFingerOn;
        }
